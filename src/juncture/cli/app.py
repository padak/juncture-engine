...
def init(
    ...
    with_examples: bool = typer.Option(
        False,
        "--with-examples",
        help="Also scaffold stg_users/user_count demo models + schema.yml tests. "
        "Default is a minimal skeleton (juncture.yaml, models/, seeds/, README.md).",
    ),
) -> None:
    """Scaffold a new Juncture project.

    The default output is a minimal skeleton so you can drop your own seeds
    and models into it without having to delete placeholders first. Pass
    ``--with-examples`` to get the old demo (``stg_users`` + ``user_count``
    + ``schema.yml``) that ``juncture run`` can execute end-to-end against
    hard-coded VALUES.
    """
    path = path.resolve()
    if name is None:
...
"""
    ...
    if with_examples:
        (path / "models" / "staging").mkdir(exist_ok=True)
        (path / "models" / "marts").mkdir(exist_ok=True)
...
"""
        ...
        readme_body = (
            f"# {name}\n\n"
            "Generated by `juncture init --with-examples`. Two demo models "
            "(`stg_users`, `user_count`) are wired up so `juncture run` "
            "works immediately.\n\n"
            "Tutorial: https://github.com/padak/juncture-engine/blob/main/docs/TUTORIAL.md\n"
        )
        next_steps = f"Next steps:\n  cd {path.name}\n  juncture run\n  juncture test"
    else:
        readme_body = (
            f"# {name}\n\n"
            "Generated by `juncture init`. To get started:\n\n"
            "1. Drop your CSV / parquet seeds into `seeds/`.\n"
            "2. Write SQL models under `models/*.sql` (or Python models "
            "with `@transform`).\n"
            "3. Run `juncture run` (and `juncture test` once you add "
        ...
        next_steps = (
            f"Next steps:\n"
            f"  cd {path.name}\n"
            f"  # add seeds under seeds/ and models under models/\n"
            f"  juncture run\n\n"
            f"Or scaffold with demo models:\n"
            f"  juncture init {path.name} --with-examples"
        )
